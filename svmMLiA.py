...
def selectJ(i,oS,Ei):
	...
	if(len(validEacheList))>1:
		for k in validEacheList:
			...
			Ek=calcEk(oS,k)
			deltaE=abs(Ei-Ek)
			if(deltaE>maxDelteaE):
				maxK=k;maxDelteaE=deltaE;Ej=Ek
		return maxK,Ej
	else:
	...
	while (iter<maxIter) and ((alphaPairsChanged>0) or (entireSet)):
		...
		if entireSet:
			for i in range(oS.m):
				...
				print "fullSet,iter:%d i:%d, pairschaged %d" %(iter,i,alphaPairsChanged)
				iter +=1
		else:
...
def testDigits(kTup=('rbf',10)):
	...
	sVs=dataMat[svInd]
	labelSV=labelMat[svInd];
	print"the are %d Support Vectors"%shape(sVs)[0]
	m,n=shape(dataMat)
	errorCount=0
	for i in range(m):
